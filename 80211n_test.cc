...
NS_LOG_COMPONENT_DEFINE("80211n");
...
class RoutingExperiment
{
  ...
  private:
    ...
    std::string m_CSVfileName{"80211n_test.csv"}; //!< CSV filename.
    int m_nSinks{1};                                      //!< Number of sink nodes.
    std::string m_protocolName{"OLSR"};                    //!< Protocol name.                                    //!< Tx power.
...
RoutingExperiment::CommandSetup(int argc, char** argv)
{
    ...
    cmd.AddValue("sinks", "Number of sink nodes", m_nSinks);
    cmd.Parse(argc, argv);
...
{
    LogComponentEnable("80211n", LOG_LEVEL_INFO);
